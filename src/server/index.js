...
try {
  ...
  console.log(
    "https certs not found in /certs (or load error), https will be unavailable:",
    e.message
  );
}
...
if (!APIKEY) {
  console.log(
    "API_KEY not set.  Set as environment var or start with `API_KEY=<value> npm run dev`. Exiting..."
  );
  process.exit(1);
}
const headers = {
